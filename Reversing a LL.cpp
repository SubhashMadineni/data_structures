...
void reverse(){
	...
	current = head;
	while(current != NULL){
		next = current->next;			//stroing current pointing variable in next so that we cannot loose it later
		current->next = prev;			//reversing the arrow of the box
		prev = current;					//shifting one step ahead each variable
		current = next;
	}
	head = prev;
	cout<<"Reversing....\n";
